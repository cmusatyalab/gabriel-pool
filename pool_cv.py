...
def process(img):
    ...
    if rtn_msg['status'] == 'success':
        rtn_msg, objects = _detect_cue(img, mask_tables, mask_balls)
        if objects is not None:
            cue = objects
    if rtn_msg['status'] == 'success':
        rtn_msg, objects = _detect_CO_balls(img, balls, cue)
        if objects is not None:
            CO_balls = objects
    if rtn_msg['status'] == 'success':
        rtn_msg, objects = _detect_pocket(img, mask_tables, cue)
        if objects is not None:
